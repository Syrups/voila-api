...
module.exports = {
	...
	dismiss: function (req, res, next) {
		...
		Proposition.findByIdAndUpdate(id, {
			...
			if (!err) {
				...
				res.status(200).json({ code: 200, message: "Proposition dismissed." });
	...
	acknowledge: function (req, res, next) {
		var id = req.params['id'];
		var now = Date.now();
		Proposition.findByIdAndUpdate(id, { receivedAt: now }, function (err, proposition) {
			if (!err) {
				res.status(200).json({ code: 200, data: proposition });
			} else {
				console.log(err);
