...
module.exports = {
	...
	create: function (req, res, next) {
		...
		crypto.pbkdf2(req.param('password'), new Buffer(salt), 1000, 25, function (err, password) {
			...
			User.create({
				...
				if (!err) {
					res.status(201).json({ code: 201, message: "User created.", id: user.id, name: user.name, avatar : user.avatar, token: token });
				} else {
					User.findOne({
						name: req.param('username')
					}, function (err, user) {
		...
		User.findOne({ name: req.param('username') }, function (err, user) {
			if (!user) {
				...
				if (crypto.pbkdf2Sync(req.param('password'), user.salt, 1000, 25) != user.password) {
					...
					res.status(200).json({ message: "Authenticated", token: user.token, id: user.id });
				}
			}
		});
	},
	...
	takenPropositions: function (req, res, next) {
		Proposition.find({ takers: req.user.id }, function (err, propositions) {
			res.status(200).json(propositions);
		})
	},

	pendingPropositions: function (req, res, next) {
		Proposition.find({ receivers: req.user.id, "$and": [
	...
	receivedPropositions: function (req, res, next) {
		Proposition.find({ receivers: req.user.id }, function (err, propositions) {
			res.status(200).json(propositions);
		})
	},

	sentPropositions: function (req, res, next) {
		Proposition.find({ sender: req.user.id }, function (err, propositions) {
			res.status(200).json(propositions);
		})
	},
